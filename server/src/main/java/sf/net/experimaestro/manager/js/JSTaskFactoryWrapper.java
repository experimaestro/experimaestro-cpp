...
public class JSTaskFactoryWrapper extends JSBaseObject {
    ...
    private final Scriptable information;

    TaskFactory factory;

    @JSFunction
    public JSTaskFactoryWrapper(XPMObject xpm, Scriptable information) {
        this.xpm = xpm;
    ...
    public List<Object> run(String plan) throws Exception {
        ...
        final TaskContext taskContext = xpm.newTaskContext().clone();
        taskContext.simulate(false);
        return JSTaskWrapper.wrap(task.runPlan(plan, true, new JSScriptRunner(this), taskContext));
    }
